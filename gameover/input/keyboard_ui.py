...
from gameover.input.ergodox_tony import large, key_str_to_vk
        ...
        with Grid(id='keyboard-grid'):
            for key_type, key_content in key_type_and_content:
                if key_type == 'long-bottom':
                    continue
                disabled = key_type == 'placeholder'
                if not disabled:
                    key_vk = key_str_to_vk.get(key_content, '')
                yield Button(key_content, classes=f'key {key_type}', disabled=disabled)
